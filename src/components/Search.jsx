...
const Search = () => {
  ...
  const mergedProductsLocal = data?.map((product) => {
    const foundInLocal = localeProducts?.find(
      ({productId}) => String(productId) === String(product?.productId),
    );

    if (foundInLocal && foundInLocal?.title.toLowerCase().includes(`${queryText}`)) {
      return {
        ...foundInLocal,
      };
    } else if (product?.title.toLowerCase().includes(`${queryText}`)) {
      return {
        ...product,
      };
    }
  ...
  const mergedProducts = mergedProductsLocal?.filter(item => item)
  ...
  useEffect(() => {
    const mergeProductsServer = data?.map((product) => {
      ...
      if (foundInCart && foundInCart?.title.toLowerCase().includes(`${queryText}`)) {
        return {
          ...foundInCart,
          userId: userIdState || null,
        };
      } else if (foundInWish && foundInCart?.title.toLowerCase().includes(`${queryText}`)) {
        return {
          ...product,
          userId: userIdState || null,
          wish: true,
    ...
    const filteredProductsServer = mergeProductsServer?.filter((item) => item)

    setGlobalServerProducts(filteredProductsServer);
